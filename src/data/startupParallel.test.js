...
describe('첫 화면 조회 병렬화', () => {
  it('어떤 조회가 끝나기 전에도 14개 테이블과 공동 충전액 조회를 모두 시작한다', async () => {
    const pending = new Map()
    from.mockImplementation((table) => {
      const response = new Promise((resolve) => pending.set(table, resolve))
      const query = { select: () => query, order: () => query, limit: () => query, then: response.then.bind(response) }
      return query
    })
